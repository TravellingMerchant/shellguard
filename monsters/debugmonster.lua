...
function init()
  ...
  self.debugParts = config.getParameter("debugParts", false)
end
...
function update(dt)
  ...
  if self.debugParts then
    for k, v in pairs(self.debugParts) do
      world.debugPoint(vec2.add(mcontroller.position(), animator.partPoint(v, "debugOffset")), hashColor(v))
    end
  end
end

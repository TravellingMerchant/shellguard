...
function sgufoairstrike.enter()
  if not hasTarget() then return nil end

  rangedAttack.setConfig(config.getParameter("sgufoairstrike.projectile.type"), config.getParameter("sgufoairstrike.projectile.config"), config.getParameter("sgufoairstrike.fireInterval"))

  return {
    timer = 0,
    bobTime = config.getParameter("sgufoairstrike.bobTime"),
    bobHeight = config.getParameter("sgufoairstrike.bobHeight"),
    skillTime = config.getParameter("sgufoairstrike.skillTime"),
...
function sgufoairstrike.enteringState(stateData)
  monster.setActiveSkillName(nil)
end

function sgufoairstrike.update(dt, stateData)
  mcontroller.controlFace(1)
  if not hasTarget() then return true end

  local projectileOffset = config.getParameter("sgufoairstrike.projectileOffset")
...
function sgufoairstrike.leavingState(stateData)
end

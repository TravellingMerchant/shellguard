...
function sgSlamAttackBlast.enter()
  if self.targetPosition == nil then return nil end

  return {
    slamHeight = config.getParameter("sgSlamAttackBlast.slamHeight"),
    riseSpeed = config.getParameter("sgSlamAttackBlast.riseSpeed"),
    slamSpeed = config.getParameter("sgSlamAttackBlast.slamSpeed"),
    timer = config.getParameter("sgSlamAttackBlast.skillTime"),
    stunDuration = config.getParameter("sgSlamAttackBlast.stunDuration"),
    prepareSlam = false,
    slam = false
  }
end

function sgSlamAttackBlast.enteringState(stateData)
  monster.setActiveSkillName("sgSlamAttackBlast")
end

function sgSlamAttackBlast.update(dt, stateData)
  ...
  if stateData.slam then
    ...
    if mcontroller.onGround() then -- and not monster.isFiring() then
      ...
      mcontroller.controlApproachYVelocity(-config.getParameter("sgSlamAttackBlast.slamSpeed"), 100)
    end
  end
...
function sgSlamAttackBlast.leavingState(stateData)
  monster.setDamageOnTouch(false)
...
function sgSlamAttackBlast.flyToTargetYOffsetRange(targetPosition)
  local position = mcontroller.position()

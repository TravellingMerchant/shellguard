...
function sgChomperFollowWait.enter()
  self.headRotationCenter = config.getParameter("sgChomperFollowWait.headRotationCenter", {0, 0})
  self.projectileSpawnOffset = config.getParameter("sgChomperFollowWait.projectileSpawnOffset", {0, 0})
  self.headAngleOffset = config.getParameter("sgChomperFollowWait.headAngleOffset", 1)
  ...
  self.holdAim = config.getParameter("sgChomperFollowWait.holdAim", false)
  self.targetAimFound = false
  self.worldFirePoint = {0, 0}
  
  self.targetAngle = 0
  
  self.angleApproach = config.getParameter("sgChomperFollowWait.angleApproach", 1)
  
  self.burstCount = config.getParameter("sgChomperFollowWait.burstCount", 1)
  self.burstTime = config.getParameter("sgChomperFollowWait.burstTime", 0.1)
  self.burstTimer = self.burstTime

  return {
    projectileType = config.getParameter("sgChomperFollowWait.projectileType", "dragonblockbuster"),
    projectileParameters = config.getParameter("sgChomperFollowWait.projectileParameters", {}),
    trackSourceEntity = config.getParameter("sgChomperFollowWait.trackSourceEntity", false)
  }
end

function sgChomperFollowWait.enteringState(stateData)  
  animator.setAnimationState("head", "attackWindup")
  animator.playSound("laserWindup")
end

function sgChomperFollowWait.update(dt, stateData)
  ...
  sgChomperFollowWait.updateHead(stateData)
end

function sgChomperFollowWait.updateHead(stateData)
...
function sgChomperFollowWait.leavingState(stateData)
end

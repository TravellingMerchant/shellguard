...
--------------------------------------------------------------------------------
function sgarenabossElectricBurstAttackrocket.initialStateData()
  return {
    riseHeight = config.getParameter("sgarenabossElectricBurstAttackrocket.riseHeight"),
    riseSpeed = config.getParameter("sgarenabossElectricBurstAttackrocket.riseSpeed"),
    firing = false,
    skillTimer = 0,
    skillDuration = config.getParameter("sgarenabossElectricBurstAttackrocket.skillDuration"),
    angleCycle = config.getParameter("sgarenabossElectricBurstAttackrocket.angleCycle"),
    fireTimer = 0,
    fireInterval = config.getParameter("sgarenabossElectricBurstAttackrocket.fireInterval"),
    fireAngle = 0,
    maxFireAngle = config.getParameter("sgarenabossElectricBurstAttackrocket.maxFireAngle"),
    projectileCount = config.getParameter("sgarenabossElectricBurstAttackrocket.projectileCount"),
    winddownTimer = config.getParameter("sgarenabossElectricBurstAttackrocket.winddownTime")
  }
end

function sgarenabossElectricBurstAttackrocket.enter()
  if not hasTarget() or currentPhase() < 4 then return nil end

  return sgarenabossElectricBurstAttackrocket.initialStateData()
end


function sgarenabossElectricBurstAttackrocket.enterWith(args)
...
--------------------------------------------------------------------------------
function sgarenabossElectricBurstAttackrocket.enteringState(stateData)
  animator.setAnimationState("movement", "idle")

  monster.setActiveSkillName("sgarenabossElectricBurstAttackrocket")

  stateData.lastToSpawn = world.distance(self.spawnPosition, mcontroller.position())
end

--------------------------------------------------------------------------------
function sgarenabossElectricBurstAttackrocket.update(dt, stateData)
  ...
  --In firing position
    ...
    if stateData.skillTimer < stateData.skillDuration then
      ...
      if stateData.fireTimer <= 0 then
        ...
        sgarenabossElectricBurstAttackrocket.fire(angle, stateData.projectileCount)
    ...
    --Wind down floating to the ground before leaving the state
      ...
      local toSpawn = world.distance(self.spawnPosition, mcontroller.position())
      if stateData.winddownTimer == config.getParameter("sgarenabossElectricBurstAttackrocket.winddownTime") then
...
function sgarenabossElectricBurstAttackrocket.fire(angle, count)
  local projectileType = config.getParameter("sgarenabossElectricBurstAttackrocket.projectile.type")
  local projectileConfig = config.getParameter("sgarenabossElectricBurstAttackrocket.projectile.config")
  local projectileCenterOffset = config.getParameter("sgarenabossElectricBurstAttackrocket.projectile.centerOffset")
  ...
  local innerRadius = config.getParameter("sgarenabossElectricBurstAttackrocket.projectile.innerRadius")
...
function sgarenabossElectricBurstAttackrocket.leavingState(stateData)

...
--------------------------------------------------------------------------------
function sgarenabosslaser.enter()
  if not hasTarget() then return nil end

  return {
    windupTimer = config.getParameter("sgarenabosslaser.windupTime"),
    winddownTimer = config.getParameter("sgarenabosslaser.winddownTime"),
    distanceRange = config.getParameter("sgarenabosslaser.distanceRange"),
    skillTimer = 0,
    skillDuration = config.getParameter("sgarenabosslaser.skillDuration"),
    angleCycle = config.getParameter("sgarenabosslaser.angleCycle"),
    fireTimer = 0,
    fireInterval = config.getParameter("sgarenabosslaser.fireInterval"),
    fireAngle = 0,
    maxFireAngle = config.getParameter("sgarenabosslaser.maxFireAngle"),
...
--------------------------------------------------------------------------------
function sgarenabosslaser.enteringState(stateData)
  animator.setAnimationState("movement", "idle")

  monster.setActiveSkillName("sgarenabosslaser")
end

--------------------------------------------------------------------------------
function sgarenabosslaser.update(dt, stateData)
  ...
  if stateData.windupTimer > 0 then
    if stateData.windupTimer == config.getParameter("sgarenabosslaser.windupTime") then
  ...
  mcontroller.controlParameters({
    walkSpeed = config.getParameter("sgarenabosslaser.moveSpeed"),
    runSpeed = config.getParameter("sgarenabosslaser.moveSpeed")  
  })
  ...
  if stateData.skillTimer > stateData.skillDuration then
    ...
    if stateData.winddownTimer > 0 then
      if stateData.winddownTimer == config.getParameter("sgarenabosslaser.winddownTime") then
  ...
  sgarenabosslaser.controlFace(dt, stateData, targetDir)
  ...
  if stateData.fireTimer <= 0 then
    ...
    sgarenabosslaser.fire(aimVector)
...
function sgarenabosslaser.controlFace(dt, stateData, direction)
  if direction ~= mcontroller.facingDirection() and stateData.facingTimer > 0 then
    stateData.facingTimer = stateData.facingTimer - dt
  else
    stateData.facingTimer = config.getParameter("sgarenabosslaser.changeFacingTime")
    mcontroller.controlFace(direction)
  end
end

function sgarenabosslaser.fire(aimVector)
  local projectileType = config.getParameter("sgarenabosslaser.projectile.type")
  local projectileConfig = config.getParameter("sgarenabosslaser.projectile.config")
...
function sgarenabosslaser.leavingState(stateData)
  animator.setAnimationState("flameSound", "off")
  animator.setAnimationState("flamethrower", "winddown")

...
--------------------------------------------------------------------------------
function sgarenabossgrenadetoss.enter()
  if not hasTarget() then return nil end

  return {
    windupTimer = config.getParameter("sgarenabossgrenadetoss.windupTime"),
    winddownTimer = config.getParameter("sgarenabossgrenadetoss.winddownTime"),
    distanceRange = config.getParameter("sgarenabossgrenadetoss.distanceRange"),
    skillTimer = 0,
    skillDuration = config.getParameter("sgarenabossgrenadetoss.skillDuration"),
    angleCycle = config.getParameter("sgarenabossgrenadetoss.angleCycle"),
    fireTimer = 0,
    fireInterval = config.getParameter("sgarenabossgrenadetoss.fireInterval"),
    fireAngle = 0,
    maxFireAngle = config.getParameter("sgarenabossgrenadetoss.maxFireAngle"),
...
--------------------------------------------------------------------------------
function sgarenabossgrenadetoss.enteringState(stateData)
  animator.setAnimationState("movement", "idle")

  monster.setActiveSkillName("sgarenabossgrenadetoss")
end

--------------------------------------------------------------------------------
function sgarenabossgrenadetoss.update(dt, stateData)
  ...
  if stateData.windupTimer > 0 then
    if stateData.windupTimer == config.getParameter("sgarenabossgrenadetoss.windupTime") then
  ...
  mcontroller.controlParameters({
    walkSpeed = config.getParameter("sgarenabossgrenadetoss.moveSpeed"),
    runSpeed = config.getParameter("sgarenabossgrenadetoss.moveSpeed")  
  })
  ...
  if stateData.skillTimer > stateData.skillDuration then
    ...
    if stateData.winddownTimer > 0 then
      if stateData.winddownTimer == config.getParameter("sgarenabossgrenadetoss.winddownTime") then
  ...
  sgarenabossgrenadetoss.controlFace(dt, stateData, targetDir)
  ...
  if stateData.fireTimer <= 0 then
    ...
    sgarenabossgrenadetoss.fire(aimVector)
...
function sgarenabossgrenadetoss.controlFace(dt, stateData, direction)
  if direction ~= mcontroller.facingDirection() and stateData.facingTimer > 0 then
    stateData.facingTimer = stateData.facingTimer - dt
  else
    stateData.facingTimer = config.getParameter("sgarenabossgrenadetoss.changeFacingTime")
    mcontroller.controlFace(direction)
  end
end

function sgarenabossgrenadetoss.fire(aimVector)
  local projectileType = config.getParameter("sgarenabossgrenadetoss.projectile.type")
  local projectileConfig = config.getParameter("sgarenabossgrenadetoss.projectile.config")
...
function sgarenabossgrenadetoss.leavingState(stateData)
  animator.setAnimationState("flameSound", "off")
  animator.setAnimationState("flamethrower", "winddown")

...
function sgmegacrystalattack.enter()
  ...
  return {
    windupTimer = 0.6,
    timer = config.getParameter("sgmegacrystalattack.skillTime", 0.3),
    winddownTimer = 0.6,
    spawnAngle = config.getParameter("sgmegacrystalattack.spawnAngle", 1.2),
    direction = util.randomDirection(),
    rotateInterval = config.getParameter("sgmegacrystalattack.rotateInterval", 0.1),
    rotateAngle = config.getParameter("sgmegacrystalattack.rotateAngle", 0.02)
  }
end

function sgmegacrystalattack.enteringState(stateData)
  animator.setAnimationState("eye", "windup")
  animator.playSound("spawnCharge")
end

function sgmegacrystalattack.update(dt, stateData)

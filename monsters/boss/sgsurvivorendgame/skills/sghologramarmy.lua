...
--------------------------------------------------------------------------------
function sghologramarmy.enter()
  if not hasTarget() then return nil end

  return {
    windupTimer = config.getParameter("sghologramarmy.windupTime"),
    winddownTimer = config.getParameter("sghologramarmy.winddownTime"),
    distanceRange = config.getParameter("sghologramarmy.distanceRange"),
    skillTimer = 0,
    skillDuration = config.getParameter("sghologramarmy.skillDuration"),
    angleCycle = config.getParameter("sghologramarmy.angleCycle"),
    fireTimer = 0,
    fireInterval = config.getParameter("sghologramarmy.fireInterval"),
    fireAngle = 0,
    maxFireAngle = config.getParameter("sghologramarmy.maxFireAngle"),
...
--------------------------------------------------------------------------------
function sghologramarmy.enteringState(stateData)
  animator.setAnimationState("movement", "idle")

  monster.setActiveSkillName("sghologramarmy")
end

--------------------------------------------------------------------------------
function sghologramarmy.update(dt, stateData)
  ...
  if stateData.windupTimer > 0 then
    if stateData.windupTimer == config.getParameter("sghologramarmy.windupTime") then
  ...
  mcontroller.controlParameters({
    walkSpeed = config.getParameter("sghologramarmy.moveSpeed"),
    runSpeed = config.getParameter("sghologramarmy.moveSpeed")  
  })
  ...
  if stateData.skillTimer > stateData.skillDuration then
    ...
    if stateData.winddownTimer > 0 then
      if stateData.winddownTimer == config.getParameter("sghologramarmy.winddownTime") then
  ...
  sghologramarmy.controlFace(dt, stateData, targetDir)
...
function sghologramarmy.controlFace(dt, stateData, direction)
  if direction ~= mcontroller.facingDirection() and stateData.facingTimer > 0 then
    stateData.facingTimer = stateData.facingTimer - dt
  else
    stateData.facingTimer = config.getParameter("sghologramarmy.changeFacingTime")
    mcontroller.controlFace(direction)
  end
end

function sghologramarmy.fire(aimVector)
  local projectileType = config.getParameter("sghologramarmy.projectile.type")
  local projectileConfig = config.getParameter("sghologramarmy.projectile.config")
...
function sghologramarmy.leavingState(stateData)
  animator.setAnimationState("flameSound", "off")
  animator.setAnimationState("flamethrower", "winddown")

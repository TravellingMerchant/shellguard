...
function update(dt)
	--Target freezing
	local targets = world.playerQuery(mcontroller.position(), 50, {
	...
	for _,target in pairs(targets) do
		if world.entityExists(target) then
			world.spawnProjectile("nebsgcryofreezingprojectile", world.entityPosition(target), entity.id(), {0,0}, false, nil)
		end
	end

	--Sine wave movement
	self.timer = self.timer + dt
  local offset = math.sin(self.timer * self.wavePeriod) * self.waveAmplitude

	mcontroller.setVelocity({0, offset})

	self.lastAngle = newAngle

  if targets[1] then
    mcontroller.controlFace(world.entityPosition(targets[1])[1])
	else

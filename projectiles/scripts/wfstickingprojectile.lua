...
function init()
	...
	self.allyActionsOnStick = config.getParameter("allyActionOnStick",{})
	self.enemyActionsOnStick = config.getParameter("enemyActionOnStick",{})
...
function update(dt)
	...
	if #targets > 0 and not self.stuckToGround then
		if world.entityExists(targets[1]) then
			...
			if config.getParameter("wfStickToTargetTime") then
				projectile.setTimeToLive(config.getParameter("wfStickToTargetTime"))
			end
		end
	end
	...
	if self.stickingTarget then
		if world.entityExists(self.stickingTarget) then
			if self.enemyAllyDistinguishing and not self.hasActioned then
				if world.entityDamageTeam(self.stickingTarget).type == "friendly" then
					for i,action in ipairs(self.allyActionOnStick or {}) do
						projectile.processAction(action)
					end
				elseif world.entityDamageTeam(self.stickingTarget).type == "enemy" then
					for i,action in ipairs(self.enemyActionOnStick or {}) do
						projectile.processAction(action)
					end
				end
				self.hasActioned = true
			elseif not self.hasActioned then
				for i,action in ipairs(self.allyActionOnStick or {}) do
					projectile.processAction(action)
				end
				for i,action in ipairs(self.enemyActionOnStick or {}) do
			...
			self.stickingTarget = nil
			for i,action in ipairs(config.getParameter("wfStickingTargetDiedActions",{})) do
				projectile.processAction(action)
			end
		end
	end

...
function destroy()
  if projectile.sourceEntity() and world.entityExists(projectile.sourceEntity()) then
    ...
    world.spawnProjectile("fireplasmarocket", mcontroller.position(), projectile.sourceEntity(), {math.cos(rotation), math.sin(rotation)}, false, { speed = 10, power = projectile.getParameter("power")})

...
TheaGunShieldCombo = WeaponAbility:new()

function TheaGunShieldCombo:init()
  self.comboStep = 1
  self.airTime = 0
...
function TheaGunShieldCombo:update(dt, fireMode, shiftHeld)
...
function TheaGunShieldCombo:windup()
...
function TheaGunShieldCombo:wait()
...
function TheaGunShieldCombo:preslash()
...
-- State: fire
function TheaGunShieldCombo:fire()
  local stance = self.stances["fire"..self.comboStep]

  self.weapon:setStance(stance)
  self.weapon:updateAim()
  
  animator.setPartTag("swoosh", "variant", math.random(1, 3))
  ...
  util.wait(stance.duration, function()
    ...
    if stance.projectile then
	  ...
	  params.power = stance.projectileDamage * config.getParameter("damageLevelMultiplier") / stance.projectileCount
	  ...
	  if stance.endWeaponRotation then
        ...
        progress = math.min(1.0, progress + (self.dt / (stance.fireTime or stance.duration)))
	  end
...
function TheaGunShieldCombo:shouldActivate()
...
function TheaGunShieldCombo:aimVector(inaccuracy)
  local aimVector = vec2.rotate({1, 0}, self.weapon.aimAngle + sb.nrand(inaccuracy, 0))
  aimVector[1] = aimVector[1] * mcontroller.facingDirection()
  return aimVector
end

function TheaGunShieldCombo:uninit()

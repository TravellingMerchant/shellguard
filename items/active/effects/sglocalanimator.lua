...
function update()
  ...
  for _, chain in pairs(self.sgchains) do
	...
	if chain.startOffset then
		chain.startPosition = vec2.add(activeItemAnimation.ownerPosition(), activeItemAnimation.handPosition(chain.startOffset))
	end
	if chain.endOffset then
		chain.endPosition = vec2.add(activeItemAnimation.ownerPosition(), activeItemAnimation.handPosition(chain.endOffset))
	end
      ...
      if chain.testCollision then
        ...
        if collision then
          ...
          if chain.bounces and chain.bounces > 0 then
            ...
            table.insert(self.wfchains, newChain)
          end

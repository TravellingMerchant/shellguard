...
function init()
  ...
  self.ignoreZeroG = config.getParameter("ignoreZeroG")
  ...
  self.damageStateDriverEmotes = config.getParameter("damageStateDriverEmotes")
	...
	self.altFireGun = config.getParameter("altFireGun")
	self.altFireProjectileType = config.getParameter("altFireProjectileType")
	self.altFireProjectileParams = config.getParameter("altFireProjectileParams")
	self.altFireInaccuracy = config.getParameter("altFireInaccuracy")
	self.altFireRate = config.getParameter("altFireRate")
	self.altFireProjectileCount = config.getParameter("altFireProjectileCount",1)
	self.altGunOffset = config.getParameter("altGunOffset")
	
	self.primaryFireHorn = config.getParameter("primaryFireHorn")
	self.primaryFireGun = config.getParameter("primaryFireGun")
	self.primaryFireProjectileType = config.getParameter("primaryFireProjectileType")
	self.primaryFireProjectileParams = config.getParameter("primaryFireProjectileParams")
	self.primaryFireInaccuracy = config.getParameter("primaryFireInaccuracy")
	self.primaryFireRate = config.getParameter("primaryFireRate")
	self.primaryFireProjectileCount = config.getParameter("primaryFireProjectileCount",1)
	self.primaryGunOffset = config.getParameter("primaryGunOffset")
...
function update()
  ...
  elseif (animator.animationState("movement") == "warpInPart1" or animator.animationState("movement") == "warpOutPart2") then
    ...
    if mcontroller.zeroG() and not self.ignoreZeroG then
      moveZeroG()
    else
      move()
    end
	...
	self.cooldown = math.max(self.cooldown - script.updateDt(),0)
	if self.primaryGunOffset then
		aimOffset = world.distance(vehicle.aimPosition("drivingSeat"),vec2.add(mcontroller.position(),vec2.rotate(vec2.mul(self.primaryGunOffset,{self.facingDirection,1}),self.angle)))
		self.primaryAimAngle = math.atan(aimOffset[2],aimOffset[1])
	end
	if self.altGunOffset then
		aimOffset = world.distance(vehicle.aimPosition("drivingSeat"),vec2.add(mcontroller.position(),vec2.rotate(vec2.mul(self.altGunOffset,{self.facingDirection,1}),self.angle)))
		self.altAimAngle = math.atan(aimOffset[2],aimOffset[1])
	end
...
function controls()
  if (vehicle.controlHeld("drivingSeat", "PrimaryFire")) and self.primaryFireHeadlight then
	...
	if (vehicle.controlHeld("drivingSeat", "PrimaryFire")) and self.primaryFireGun and self.cooldown <= 0 then
		fireProjectile(self.primaryFireProjectileType,self.primaryFireProjectileParams,self.primaryFireInaccuracy,vec2.add(mcontroller.position(),vec2.rotate(vec2.mul(self.primaryGunOffset,{self.facingDirection,1}),self.angle)),self.primaryFireProjectileCount,self.primaryFireRate,self.primaryAimAngle)
	end
  ...
  if (vehicle.controlHeld("drivingSeat", "AltFire")) and self.altFireHorn then
	...
	if (vehicle.controlHeld("drivingSeat", "AltFire")) and self.altFireGun and self.cooldown <= 0 then
		fireProjectile(self.altFireProjectileType,self.altFireProjectileParams,self.altFireInaccuracy,vec2.add(mcontroller.position(),vec2.rotate(vec2.mul(self.primaryGunOffset,{self.facingDirection,1}),self.angle)),self.altFireProjectileCount,self.altFireRate,self.altAimAngle)
	end
end
...
function fireProjectile(projectileType, projectileParams, inaccuracy, firePosition, projectileCount, cooldown, aimAngle)
  local params = sb.jsonMerge(self.projectileParameters, self.projectileParams or {})

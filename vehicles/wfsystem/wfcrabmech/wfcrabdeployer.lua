...
function init()
	...
	if not config.getParameter("wfCrabDeploymentData") then
		self.vehicleState = VehicleUnconfigured
	elseif config.getParameter("filled") then
...
function activate(fireMode, shiftHeld)
	if config.getParameter("filled") and config.getParameter("wfCrabDeploymentData") then

		if fireMode == "primary" or fireMode == "alt" then
			if self.vehicleState == VehiclePlaceable then
				animator.playSound("placeOk")

				local vehicleParams = util.mergeTable({
					ownerKey = config.getParameter("key"),
					startHealthFactor = config.getParameter("vehicleStartHealthFactor"),
					fromItem = true,
					commander = activeItem.ownerEntityId()
					--autopilot = fireMode == "alt" and true or false
				},config.getParameter("wfCrabDeploymentData",{}))
...
function update(dt)
	if self.vehicleState ~= VehicleUnconfigured then
...
function updateIcon()
	if not config.getParameter("wfCrabDeploymentData") then
		animator.setAnimationState("controller", "unconfigured")
		activeItem.setInventoryIcon(config.getParameter("unconfiguredInventoryIcon"))
	elseif config.getParameter("filled") then

...
function damageTiles()
	if self.driver then
		...
		if world.lineTileCollision(vec2.add(damageSource, self.damageRadius), vec2.add(damageSource, -self.damageRadius)) then
		  world.damageTileArea(damageSource, self.damageRadius, "foreground", damageSource, "blockish", damageFactor)
		end
		world.debugPoly({vec2.add(damageSource, self.damageRadius), vec2.add(damageSource, -self.damageRadius)}, "red")
	end
end
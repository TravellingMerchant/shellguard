...
function init()
  ...
  self.turretProjectileType = config.getParameter("turretProjectileType")
  self.turretProjectileParameters = config.getParameter("turretProjectileParameters", {})
  self.turretProjectileParameters.power = config.getParameter("turretPower", 2)
  self.turretFireTime = config.getParameter("turretFireTime", 2)
  self.turretFireTimeCooldown = 0
  self.turretInaccuracy = config.getParameter("turretInaccuracy", 2)
  self.turretAngleRange = util.toRadians(config.getParameter("turretAngleRange", 45))
  self.turretAngle = 0
  ...
  -- Here goes nothing --
  self.damageRadius = config.getParameter("damageRadius", 2)
  self.damageFactor = config.getParameter("damageRadius", 2)
  self.isRamModeEnabled = false
  self.ramModCanToggle = true
end
...
function update(dt)
  ...
  elseif (animator.animationState("movement") == "warpInPart1" or animator.animationState("movement") == "warpOutPart2") then
	...
	if self.isRamModeEnabled then
		damageTiles()
	end
  end
  
  self.turretFireTimeCooldown = math.max(self.turretFireTimeCooldown - dt, 0)
end
...
function controls()
  if (vehicle.controlHeld("drivingSeat", "PrimaryFire")) then
    if self.turretFireTimeCooldown == 0 then
      local rotation = self.turretAngle
      local aimVector = {math.cos(rotation) * self.facingDirection, math.sin(rotation)}
      aimVector = vec2.rotate(aimVector, self.angle)
      aimVector = vec2.rotate(aimVector, sb.nrand(self.turretInaccuracy, 0))
      
      world.spawnProjectile(self.turretProjectileType, firePosition(), entity.id(), aimVector, false, self.turretProjectileParameters)
      animator.playSound("fire")
      
      self.turretFireTimeCooldown = self.turretFireTime
    end
  end
  ...
  if (vehicle.controlHeld("drivingSeat", "AltFire")) then
	if self.ramModCanToggle then
		self.isRamModeEnabled = not self.isRamModeEnabled
		animator.setAnimationState("ramMode", self.isRamModeEnabled and "on" or "off")
		self.ramModCanToggle = false
	end
  else
	self.ramModCanToggle = true
  end
...
function animate()
  ...
  local targetPosition = vehicle.aimPosition("drivingSeat")
  local rotationOffset = animator.partProperty("gun", "offset")
  targetPosition = world.distance(targetPosition, {rotationOffset[1], rotationOffset[2]})
  
  local toTarget = world.distance(targetPosition, mcontroller.position())
  local targetAngle = math.atan(toTarget[2], toTarget[1] * self.facingDirection) - self.angle * self.facingDirection
  self.turretAngle = util.clamp(targetAngle, -self.turretAngleRange, self.turretAngleRange)
  
  animator.resetTransformationGroup("weapon")
  animator.rotateTransformationGroup("weapon", self.turretAngle, animator.partProperty("gun", "offset"))
  
  --world.debugLine(mcontroller.position(), targetPosition, "blue")
...
function damageTiles()
	if self.driver then
		...
		local damageSource = vec2.add(position, animator.partPoint("piercer", "ramDamageCenter"))
		local damageFactor = math.abs(mcontroller.xVelocity()) * self.damageFactor
		local damageArea = {
			vec2.add(damageSource, {-math.sqrt(2) / 2.0 * self.damageRadius, -math.sqrt(2) / 2.0 * self.damageRadius}),
			vec2.add(damageSource, {math.sqrt(2) / 2.0 * self.damageRadius, math.sqrt(2) / 2.0 * self.damageRadius})
		}
		if world.rectTileCollision(mcontroller.collisionBoundBox(), {"Block"}) then
		  world.damageTileArea(damageSource, self.damageRadius, "foreground", damageSource, "blockish", damageFactor)
		end
	end
end

function firePosition()
  local animationPosition = vec2.div(config.getParameter("animationPosition", {0, 0}), 8)
  local fireOffset = {0,0}
  
  fireOffset = vec2.add(animationPosition, animator.partPoint("gun", "projectileSource"))
  return vec2.add(mcontroller.position(), fireOffset)
end